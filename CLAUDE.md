...
- **symbols/** — Source code symbol extraction. Uses tree-sitter query patterns + language-specific hooks for most languages (TS/JS, Go, Rust, Python, Java, Kotlin, C, C++, C#, Swift, PHP, Scala, Zig, Protobuf, Bash, R, Haskell), regex for Perl, Ruby, Elixir, and COBOL. Parallelized across packages and within packages with rayon. Tree-sitter queries compiled once per language via `OnceLock`. All extractors produce the same `SymbolInfo` struct (uses `Arc<str>` for file paths).
