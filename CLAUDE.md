...
- **index/** — Build orchestrator. Walks the repo, discovers manifests, parses them via the `ManifestParser` trait (one impl per ecosystem: npm, go, cargo, python, maven, gradle, perl, ruby, nix), extracts symbols, writes to SQLite. Builds are incremental at file granularity via SHA-256 content hashing (`file_hashes` table). Freshness is decided by things that move when the working tree changes: the repo-wide file-tree hash over (path, size), and a per-package pre-check (`hash::package_source_staleness`) that compares source-file mtimes AND the on-disk path set against the paths stored in `file_hashes`. The contract that buys is precise: a package is re-hashed when any of its files changes **mtime, size, or path** (so a rename, a touch, or any edit that changes length is caught, and an mtime moving backwards counts as a change). An edit that changes none of the three — a same-size in-place rewrite that also preserves mtime, i.e. `cp -p`/`install -p` from a build cache, `rsync -a`, a `tar -x` restore, a formatter that keeps byte length — is **not** detected, and needs `shire build --force`. Ordinary editors move the mtime and are unaffected. The per-package "needs a re-check" signal that `phase_index_files` derives is persisted to `shire_meta.pending_source_recheck` in that phase's transaction and cleared only when the extraction transaction commits, and then only for the packages that were actually re-checked (one whose walk failed stays flagged), so neither a build that dies in between nor a package that could not be read can lose it. `.git/index` is never used to gate a build — an ordinary edit does not touch it. A package whose extraction fails (an unwalkable directory) keeps its existing rows — including its `files` rows, which the file walk deliberately does not delete for a path it could not read, since `symbol_refs` key on `files.id` and the orphan sweep would take them with it, and including its *package* row, since the manifest walk records the same unreadable paths and `hold_back_unreadable_removals` keeps a manifest that was merely invisible out of `phase_remove_deleted`; `build_index` (the CLI) reports it as a non-zero exit, while `build_index_quiet` (MCP on-demand rebuild, watch daemon) only logs it, because the build itself did commit. Single-pass read+hash+extract avoids double file reads. The file tree walk is capped at 500k files, and hitting the cap counts as a blind spot over the whole tree (a truncated walk proves nothing about what is absent): nothing is deleted, no file-tree hash is stored, and the build warns on stderr. A build holds an advisory `flock` on `<db_path>.lock` for its whole run (`index::lock`), so two builders cannot both take the insert-only full-build path and double every symbol. `shire clean` takes the same lock and refuses while a build is running, and never removes the lock file (unlinking it while a builder holds it lets a second builder take the lock at the same path). The lock is taken only after `db::guard` has accepted the `db_path`, since the lock file's path is derived from a repo-controlled `shire.toml` by string concatenation: a symlink, a non-SQLite file, or a SQLite database with tables of its own and no `shire_meta` is refused before the lock, while a database that cannot be inspected (a concurrent build holds it under `journal_mode=MEMORY`) is re-checked under the lock and only then refused — outside `<repo>/.shire/` and `~/.claude/shire/`, where an unreadable index is instead rebuilt. `db::create_schema` runs in one transaction so an interrupted first build cannot leave a half-schema without `shire_meta`. Callers pick what happens on a conflict: `build_index` (CLI) and `build_index_quiet_waiting` (the watch daemon, whose trigger names a specific batch of edits, and `serve`'s startup recovery from a corrupt index) wait out the competing build — up to `lock::LOCK_TIMEOUT`, minutes rather than seconds, because what is being waited on is a whole build — then report the conflict; only `build_index_quiet` (the MCP server's per-tool-call check, which comes round again next call) skips — returning `BuildOutcome::Skipped`, which the server must not mistake for a rebuild, or the debounce never applies. All walks honour committed `.gitignore` files but deliberately not the machine-local `core.excludesFile`/`.git/info/exclude`, so the index does not depend on the machine that built it (`ignore_walk_builder` in `src/index/mod.rs` holds those settings; `symbols::walker` matches them). Two manifests of different ecosystems in one directory collapse to one package row (`packages.path` is UNIQUE); the drop is logged, and when a removal leaves a directory with no package at all the stored hashes of the manifests still on disk there are cleared so the next build re-parses them. Detects proto→generated-code boundary edges during file walking using stem matching and package-scope filtering.
- **symbols/** — Source code symbol extraction. Uses tree-sitter query patterns + language-specific hooks for most languages (TS/JS, Go, Rust, Python, Java, Kotlin, Dart, C, C++, C#, Swift, PHP, Scala, Zig, Protobuf, Bash, R, Haskell, YAML, SQL, HCL, TOML, Perl, Ruby, OCaml, Lua, Elixir, Clojure, Erlang, Julia, Gleam, Odin, Nix, Nim), regex for COBOL. Parallelized across packages and within packages with rayon. Tree-sitter queries compiled once per language via `OnceLock`. All extractors produce the same `SymbolInfo` struct (uses `Arc<str>` for file paths). Cross-reference extraction (call, type, import, impl) is supported for 8 tier-1 languages: Go, Python, Java, TypeScript, JavaScript, Perl, Ruby, Scala — captured via `@reference.*` tree-sitter captures and written to the `symbol_refs` table. The source-file walker (`symbols::walker`) applies `discovery.exclude` in addition to its own hardcoded excludes, honors only committed `.gitignore`/`.ignore` files (never a personal global gitignore or `.git/info/exclude`), and never aborts a build over a malformed ignore-file pattern — it logs a warning and keeps walking. An *I/O* error is the opposite case and does abort the walk: an unreadable directory is a blind spot, and returning the short file list instead would make the caller delete the package's symbols, references and file hashes for everything under it.
- **db/** — SQLite with WAL mode (a 5 s `busy_timeout` on every connection; readers never set the journal mode, which is a property of the file and a write). Builds run under `journal_mode=MEMORY` for throughput and set a `build_in_progress` marker first, so an interrupted build is detected on the next open: `open_or_create` runs `PRAGMA quick_check`, and a corrupt index is deleted (with its WAL/SHM sidecars) and rebuilt from scratch rather than dead-ending the user. FTS5 full-text search (packages, symbols, files, docs) with custom tokenizers (`unicode61` with `tokenchars` for underscores/hyphens) and `prefix='2,3'` indexes on `packages_fts`/`docs_fts` (`symbols_fts` deliberately has none: measured as no query gain for +28% database size). `symbols_fts` has a single definition (`db::SYMBOLS_FTS_DDL`/`SYMBOLS_FTS_TRIGGERS`) shared with the build's phase-7 bulk rebuild, and carries a `name_tokens` column holding the identifier's sub-tokens (`verifyJwtToken` → `verify jwt token`, via `db::split_identifier`), populated in the same batched INSERT as the symbol. Queries are built by `queries::fts_match_expr`: one quoted, prefix-matched phrase per whitespace token, ANDed; symbol search scopes the MATCH to `{name name_tokens}` (`queries::SYMBOL_MATCH_COLUMNS`) so a query cannot match a symbol through its signature or file path, falling back to `{name}` on a pre-v8 index that has no `name_tokens` column (naming a missing column is an FTS5 error, and a read-only `serve` never migrates). Every list-returning query takes a `limit` enforced in SQL (`queries::MAX_ROWS` ceiling). FTS triggers dropped/recreated during bulk operations for performance. Schema versioned via `shire_meta`; on a version bump `apply_schema` runs the destructive migration steps (drops, `ALTER TABLE ... ADD COLUMN`) *before* `create_schema`, then backfills and rebuilds the FTS tables. Read-only connections use `query_only` and `prepare_cached`. The `symbol_refs` table stores cross-reference records (call, type, import, impl) with B-tree indexes on `name`, `file_id`, and `enclosing_symbol`. Each ref stores a compact `file_id INTEGER REFERENCES files(id)` instead of a duplicated path string — `phase_index_files` runs before symbol extraction so the `files` table is populated when refs are inserted, and read queries JOIN `files` to resolve `file_path`. Records are rebuilt at file granularity alongside symbol extraction.
- **mcp/** — MCP server over stdio using the `rmcp` crate. 17 tools + 2 prompt templates for semantic codebase exploration. Supports on-demand reindexing via `serve --root`: `is_stale` is purely a debounce — once `serve.debounce_s` (default 5s) has elapsed since `indexed_at`, the working tree is re-checked by running the incremental build, which is its own freshness oracle (file-tree hash, per-package mtimes, per-file content hashes) and costs ~60-200ms when nothing changed. `.git/index` is deliberately not consulted: an ordinary edit never touches it, so gating on it froze the served index for the life of the process. The check and the rebuild run under `rebuild_lock`, so concurrent tool calls wait for the in-flight build instead of racing. Every list-returning tool takes a `limit` resolved by `resolve_limit` (default 20 for searches, 100 for listings and the four reference tools; `limit: 0` means "use the default"; hard ceiling `queries::MAX_ROWS` = 200 for every tool). `json_result` fetches one row past the limit and, when it comes back, returns a `{results, truncated, limit, max, note}` envelope instead of the bare array, so a capped list is never presented as a complete one and the output stays parseable JSON. Four reference tools: `symbol_references` (all refs to a name), `symbol_callers` (call-site callers), `symbol_callees` (outbound call graph), `change_impact` (refs + dep graph for blast-radius analysis). Two boundary tools: `schema_consumers` (proto → generated), `generated_from` (generated → proto). The `reference_audit` prompt guides refactor-safety analysis.
